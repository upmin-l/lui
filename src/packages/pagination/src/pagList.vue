<template>
  <li v-for="(item,index) of pageList"
      :key="`page-item-${index}`"
      :class="[{'l-page-item':item.type === 'number'},{ 'l-pageItem-active': item.index === currentPage },{'l-pageItem-disabled':disabled}]"
      @click="handledClick(item.index)"
  >
    <span class="l-page-text" v-if="item.type === 'number'">{{ item.index }}</span>
    <i v-else>...</i>
  </li>
</template>
...
const pageList = ref([]);
const handledClick = (num => {
  if (props.disabled || num === "") return;
  emit("click", "page", num);
})
